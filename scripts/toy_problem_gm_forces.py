...
if __name__ == "__main__":
    ...
    tethered_manipulator = TetheredManipulationWithForces(controllable_chain, passive_chain, G, stiffness, damping)

    # Optimize the control law to minimize errors and calculate forces
    deltas, final_error = tethered_manipulator.control_law_with_forces_optimized(
        thetas[0], thetas[1], thetas[2], theta3_dot, theta3_desired
    )

    # Print the results
    print(f"Optimized control inputs (deltas): {deltas}")
    print(f"Final error: {final_error}")

    # Plot the kinematic chains
